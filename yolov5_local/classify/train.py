...
def train(opt, device):
    ...
    with torch_distributed_zero_first(LOCAL_RANK), WorkingDirectory(ROOT):
        if Path(opt.model).is_file() or opt.model.endswith(".pt"):
            ...
            m = hub.list("ultralytics/yolov5_local")  # + hub.list('pytorch/vision')  # models
    ...
    if RANK in {-1, 0} and final_epoch:
        LOGGER.info(
            ...
            f"\nPyTorch Hub:     model = torch.hub.load('ultralytics/yolov5_local', 'custom', '{best}')"
...
def run(**kwargs):
    ...
    Example: from yolov5_local import classify; classify.train.run(data=mnist, imgsz=320, model='yolov5m')
    """
    opt = parse_opt(True)
